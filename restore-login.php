...
<body>
    <div class="main-wrapper">
        ...
        <main>
            <div class="container">
                ...
                <?php  if(isset($_SESSION['success_restore_login'])){
                            echo $_SESSION['success_restore_login'];
                            unset($_SESSION['success_restore_login']);
                    } ?>

                <div class="wrap-content__restore">
                    <div class="wrap-forms__restore">
                        <h2 class="headline__restore">Відновлення логіну</h2>
                        <p class="descript__restore">Не повідомляйте нікому свої особисті дані, це може бути небезпечно
                            для
                            вашого профілю</p>
                        <form action="core/restore-login.php" method="post" class="form__restore">
                            <div class="wrap-input__restore">
                                <p>Ваш номер телефону</p>
                                <input type="tel" class="input__restore" name="restore_phone" id="restore_phone"
                                    placeholder="Введіть ваш номер телефону" required>
                                <div class="restore-phone_error"></div>
                            </div>
                            <div class="wrap-input__restore">
                                <p>Ваш пароль</p>
                                <input type="password" class="input__restore" name="restore_pass" id="restore_pass"
                                    placeholder="Введіть ваш пароль" required>
                                <div class="restore-pass_error"></div>
                            </div>

                            <button class="submit__restore" name="submit__restore">Відновити логін</button>

                        </form>
                    </div>
                    <img src="img/restore-bg.svg">
                </div>
            </div>
        </main>
    ...
    $("input[name~='restore_phone']").on("click", function() {
        $(this).val("+380");
    });

    $("input[name~='restore_pass']").on("input", function() {
        $.ajax({
            url: "core/restore-login.php",
            method: "POST",
            data: {
                restore_pass: $(this).val(),
                phone_check: $("input[name~='restore_phone']").val()
            },
            success: function(data) {
                $(".restore-pass_error").html(data);
            }
        });
    });

    $("input[name~='restore_phone']").on("input", function() {
        $.ajax({
            url: "core/restore-login.php",
            method: "POST",
            data: {
